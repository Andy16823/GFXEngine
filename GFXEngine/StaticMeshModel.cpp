...
void GFXEngine::Graphics::StaticMeshModel::draw(Renderer& renderer, Camera& camera, uint32_t imageIndex, std::function<void(Renderer& renderer, Camera& camera, uint32_t imageIndex)> callback) const
{
	for (size_t i = 0; i < m_meshes.size(); i++) {
		const auto& material = m_materials[i % m_materials.size()];
		material.bind(renderer, camera, imageIndex);
		if (callback) {
			callback(renderer, camera, imageIndex);
		}
